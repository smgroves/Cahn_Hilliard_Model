...
function laplace(a, nxt, nyt)
    global xright
    global xleft
    lap_a = Array{Float64}(undef, nxt, nyt)
...
function source(c_old; nx=nx, ny=ny, dt=dt)
    src_mu = Array{Float64}(undef, nx, ny)
    src_c = Array{Float64}(undef, nx, ny)
...
function relax(c_new, mu_new, su, sw, nxt, nyt; c_relax=c_relax, xright=xright, xleft=xleft, dt=dt, Cahn=Cahn)
    ...
    for iter in 1:c_relax
        for i in 1:nxt
            for j in 1:nyt
                if i > 1 && i < nxt - 1 #changed
                    x_fac = 2.0
                else
                    x_fac = 1.0
                end
                if j > 1 && j < nyt - 1 #changed
                    y_fac = 2.0
                else
                    y_fac = 1.0
                end
...
function nonL(c_new, mu_new, nxt, nyt; dt=dt, Cahn=Cahn)
    ru = Array{Float64}(undef, nxt, nyt)
    rw = Array{Float64}(undef, nxt, nyt)
...
function error2(c_old, c_new, mu, nxt, nyt; dt=dt)
    rr = Array{Float64}(undef, nxt, nyt)
    x = 0.0
    for i in 1:nxt
        for j in 1:nyt
...
function write(max_it, max_it_CH)
    ...
    open("/Users/smgroves/Documents/GitHub/Cahn_Hilliard_Model/plotting/comp_time_comparison/Job_specs_all_py_c_julia.csv", "a", lock=false) do f
