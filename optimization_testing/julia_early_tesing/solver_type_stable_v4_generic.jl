...
global out_dir = "/Users/smgroves/Documents/GitHub/Cahn_Hilliard_Model/julia/test_256/"
...
function relax(c_new, mu_new, su, sw, nxt, nyt; c_relax=c_relax, xright=xright, xleft=xleft, dt=dt, Cahn=Cahn)
    ...
    for iter in 1:c_relax
        for i in 1:nxt
            for j in 1:nyt
                ...
                open("$(out_dir)/f_full_initial_$(suffix).txt", "a", lock=false) do file
                    for i in 1:2
                ...
                open("$(out_dir)/sw_$(suffix).txt", "a", lock=false) do file
                    Printf.@printf(file, "%16.15f ", sw[i, j])
                end

                open("$(out_dir)/su_$(suffix).txt", "a", lock=false) do file
                ...
                det = a[1] * a[4] - a[2] * a[3]
                open("$(out_dir)/cnew_before_update_$(suffix).txt", "a", lock=false) do file
                    Printf.@printf(file, "%16.15f ", c_new[i, j])
                end

                open("$(out_dir)/mu_new_before_update_$(suffix).txt", "a", lock=false) do file
                    Printf.@printf(file, "%16.15f ", mu_new[i, j])
                end

                c_new[i, j] = (a[4] * f[1] - a[2] * f[2]) / det
                mu_new[i, j] = (-a[3] * f[1] + a[1] * f[2]) / det
                open("$(out_dir)/a_v5_full_$(suffix).txt", "a", lock=false) do file
                    for i in 1:4
                        Printf.@printf(file, "%16.15f ", a[i])
                    end
                    println(file)
                end
                open("$(out_dir)/f_v5_full_$(suffix).txt", "a", lock=false) do file
                    for i in 1:2
                        Printf.@printf(file, "%16.15f ", f[i])
                    end
                    println(file)
                ...
                open("$(out_dir)/cnew_after_update_$(suffix).txt", "a", lock=false) do file
                    Printf.@printf(file, "%16.15f ", c_new[i, j])
                end

                open("$(out_dir)/mu_new_after_update_$(suffix).txt", "a", lock=false) do file
                    Printf.@printf(file, "%16.15f ", mu_new[i, j])
                end

                open("$(out_dir)/d2f_$(suffix).txt", "a", lock=false) do file
            ...
            open("$(out_dir)/cnew_$(suffix).txt", "a", lock=false) do file
                println(file)
            end
            open("$(out_dir)/cnew_after_update_$(suffix).txt", "a", lock=false) do file
                println(file)
    ...
    open("$(out_dir)/a_v4_$(suffix).txt", "a", lock=false) do file
        for i in 1:4
            Printf.@printf(file, "%16.15f ", a[i])
        end
        println(file)
    ...
    open("$(out_dir)/f_v4_$(suffix).txt", "a", lock=false) do file
        for i in 1:2
            Printf.@printf(file, "%16.15f ", f[i])
        end
        println(file)
...
function vcycle(uf_new, wf_new, su, sw, nxf, nyf, ilevel)
    global n_level
    # print_mat("$(out_dir)/uf_new_1_$(version)_$(suffix).csv", uf_new)
    # open("$(out_dir)/a_v4_full_$(suffix).txt", "a", lock=false) do file
    #     Printf.@printf(file, "First relax %d %d \n", ilevel, nxf)
    # end
    # open("$(out_dir)/mu_new_after_relax0_$(suffix).txt", "a", lock=false) do file
    # Printf.@printf(file, "First relax %d %d \n", ilevel, nxf)
    # end
    uf_new, wf_new = relax(uf_new, wf_new, su, sw, nxf, nyf, c_relax=c_relax, xright=xright,
        xleft=xleft, dt=dt, Cahn=Cahn)
    # print_mat("$(out_dir)/uf_new_2_$(version)_$(suffix).csv", uf_new)

    if ilevel < n_level
        nxc = trunc(Int64, nxf / 2)
        nyc = trunc(Int64, nyf / 2)
        uc_new, wc_new = restrict_ch(uf_new, wf_new, nxc, nyc)
        # print_mat("$(out_dir)/uf_new_3_$(version)_$(suffix).csv", uf_new)
        ...
        # print_mat("$(out_dir)/uf_new_4_$(version)_$(suffix).csv", uf_new)
        # open("$(out_dir)/mu_new_after_relax0_$(suffix).txt", "a", lock=false) do file
        # Printf.@printf(file, "Second relax %d %d \n", ilevel, nxf)
        # end
        # open("$(out_dir)/f_v4_full_$(suffix).txt", "a", lock=false) do file
        #     Printf.@printf(file, "Second relax %d %d \n", ilevel, nxf)
        # end
        uf_new, wf_new = relax(uf_new, wf_new, su, sw, nxf, nyf, c_relax=c_relax, xright=xright,
            xleft=xleft, dt=dt, Cahn=Cahn)
        # print_mat("$(out_dir)/uf_new_5_$(version)_$(suffix).csv", uf_new)

    end

    # print_mat("$(out_dir)/uf_new_6_$(version)_$(suffix).csv", uf_new)
...
function error2(c_old, c_new, mu, nxt, nyt; dt=dt)
    ...
    # print_mat("$(out_dir)/error2/mu_$(version)_$(suffix).csv", mu)
    # print_mat("$(out_dir)/error2/c_old_$(version)_$(suffix).csv", c_old)
    # print_mat("$(out_dir)/error2/c_new_$(version)_$(suffix).csv", c_old)
    # print_mat("$(out_dir)/error2/rr_1_$(version)_$(suffix).csv", rr)
    ...
    # print_mat("$(out_dir)/error2/rr_2_$(version)_$(suffix).csv", rr)
    ...
    res2 = sqrt(x / (nxt * nyt))
    print_mat("$(out_dir)/error2/res2_$(version)_$(suffix).csv", res2)
...
function cahn(c_old, c_new, mu; nx=nx, ny=ny, dt=dt, max_it_CH=10000, tol=1e-10)
    ...
    # print_mat("$(out_dir)/sc_$(version)_$(suffix).csv", sc)

    while resid2 > tol && it_mg2 < max_it_CH

        c_new, mu = vcycle(c_new, mu, sc, smu, nx, ny, 1)
        print_mat("$(out_dir)/mu_$(version)_$(suffix).csv", mu)
        print_mat("$(out_dir)/c_new_$(version)_$(suffix).csv", c_new)
...
function main(max_it, max_it_CH)
    ...
    nc = copy(oc)
    # open("$(out_dir)/phi_$(nx)_$(max_it).txt", "w", lock=false) do f
    #     writedlm(f, oc, " ")
    # end
    for it in 1:max_it
        ...
        oc = copy(nc)
        # if it % ns == 0
        #     open("$(out_dir)/phi_$(nx)_$(max_it).txt", "a", lock=false) do f
        #         writedlm(f, oc, " ")
        #     end
        # println(it)
        # end
