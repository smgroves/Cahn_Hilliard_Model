...
{ /* Prints the conserved scalar field phi as a space-delimited file */
    FILE *fphi;
    fphi = fopen("phi_convergence.m", "a");
...
{ /* Initialize the domain for convergence test; Equation 26 of Mathematics 8:97 (2020) */
    ...
    } /* Sinusoidal input for convergence test */
}
...
int main()
{
    ...
    fphi = fopen("phi_convergence.m", "w");
    fclose(fphi);
    ...
    { /* Run Cahn-Hilliard solver */
        ...
        } /* Every ns iterations print out counts to phi_convergence.m*/
        printf(" %d \n", it);
    }
    return 0;
}