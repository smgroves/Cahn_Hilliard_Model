...
{ /* Initialize the domain for convergence test; Equation 26 of Mathematics 8:97 (2020) */
    ...
    ijloop
    {
        ...
        // circular initial conditions at center
        if ((i - round(nx / 2)) * (i - round(nx / 2)) + (j - round(ny / 2)) * (j - round(ny / 2)) < CPC_width * CPC_width)
        {
            phi[i][j] = 1;
        }
...
int main()
{
    ...
    { /* Run Cahn-Hilliard solver */
        ...
        if (it % ns == 0)               /* every ns steps, print data to file */
        {
            count++;
            print_data(oc);
