...
{ /* Prints the conserved scalar field phi as a space-delimited file */
    FILE *fphi;
    fphi = fopen("../outputs/phi_CPC_big_from_file_128x128_100s_10max.m", "a");
...
int main()
{
    ...
    readCSVIntoMatrix(oc, "../data/10_16_23_CPC_relaxed_RefModel_128x64_10_16_23_relaxed_RefModel_Mps1_phos_Plk1a_20Pac_transactiv_100_128x128100s_10max.csv");
    // initialization(oc);
    show_mat(oc, nx, ny);
    mat_copy(nc, oc, 1, nx, 1, ny); /* Initialize oc and copy oc to nc */
    fphi = fopen("../outputs/phi_CPC_big_from_file_128x128_100s_10max.m", "w");
    fclose(fphi);
