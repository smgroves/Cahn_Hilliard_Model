...
void cahn(double **c_old, double **c_new, int max_it_CH, double tol)
...
int main(int argc, char *argv[])
{
    ...
    // arguments: max_it, max_it_CH
    int it = 1, max_it, ns, count = 1, it_mg = 1, max_it_CH;
    double **oc, **nc, resid2 = 1.0, tol = 1.0e-6;
    max_it = atoi(argv[1]);
    max_it_CH = atoi(argv[2]);
    ...
    printf("Total time: %f \n", cpu_time_used);
    const char *filename = "Job_specs.csv";

    // Check if the file exists
    if (access(filename, F_OK) == -1)
    {
        // File doesn't exist, perform one action (e.g., create and write)
        FILE *file = fopen(filename, "w");

        fprintf(file, "time, max_it, nx, ny, c_relax, max_it_CH, ms, dt, gam, Cahn, h, n_level \n");
        fprintf(file, "%f, ", cpu_time_used);
        fprintf(file, "%d, ", max_it);
        fprintf(file, "%d, ", nx);
        fprintf(file, "%d, ", ny);
        fprintf(file, "%d, ", c_relax);
        fprintf(file, "%d, ", max_it_CH);
        fprintf(file, "%d, ", ns);
        fprintf(file, "%f, ", dt);
        fprintf(file, "%f, ", gam);
        fprintf(file, "%f, ", Cahn);
    ...
    {
        // File exists, perform another action (e.g., append)
        FILE *file = fopen(filename, "a");

        // Print the elapsed time to the file
        fprintf(file, "%f, ", cpu_time_used);
        fprintf(file, "%d, ", max_it);
        fprintf(file, "%d, ", nx);
        fprintf(file, "%d, ", ny);
