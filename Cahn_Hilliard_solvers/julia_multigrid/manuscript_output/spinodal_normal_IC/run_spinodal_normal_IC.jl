...
# nx = 1024
outdir = "/Users/smgroves/Documents/GitHub/Cahn_Hilliard_Model/Cahn_Hilliard_solvers/julia_multigrid/manuscript_output/spinodal_normal_IC/output"
# mean_ = 0
# std_dev = 0.2
...
outdir = "/Users/smgroves/Documents/GitHub/Cahn_Hilliard_Model/Cahn_Hilliard_solvers/julia_multigrid/manuscript_output/spinodal_normal_IC/output/dt_vs_tol_num_vcycles"
m = 8
epsilon = m * (1 / 128) / (2 * sqrt(2) * atanh(0.9))
for nx in [64, 128, 256]
    ...
    for tol in [1e-5, 1e-6, 1e-7, 1e-8, 1e-9, 1e-10]
        for dt in [1e-7 * h^2, 1e-6 * h^2, 1e-5 * h^2, 1e-4 * h^2, 1e-3 * h^2, 1e-2 * h^2, 1e-1 * h^2]
            ...
            time_passed = @elapsed multigrid_solver(phi, nx, tol, outdir, dt=dt, epsilon=epsilon, max_it=max_it, max_it_CH=10000, print_mass=false, print_e=false, overwrite=false, print_r=true, print_phi=false, suffix="dt_$(dt)_mean_0_sd_0.2", check_dir=false)
            # open("/Users/smgroves/Documents/GitHub/Cahn_Hilliard_Model/Cahn_Hilliard_solvers/Job_specs.csv", "a", lock=false) do f
            #     writedlm(f, [date_time "spinodal_normal_IC_no_printing" "Julia" nx epsilon dt tol max_it 10000 time_passed], ",")
            # end
        end
