...
yy = [equilibrium_interface((x-(interface_location)*h)/gam, param_dict[alpha][0],param_dict[alpha][1],param_dict[alpha][2]) for x in xx]
...
def initialization_from_function_v2(nx, ny, beta, phi_1, phi_2, R0=0.1, gam=0.01):
    ...
    phi = (phi_2 + phi_1 * np.exp((R - R0) / (beta * gam))) / (1 + np.exp((R - R0) / (beta * gam)))
...
for alpha in [-0.5, -0.1, 0, 0.1, 0.5]:
    ...
    full_df = pd.concat([full_df, pd.DataFrame({"x":xs, "y":phi_slice,"alpha":[alpha]*len(xs)})])
...
plt.grid(True)
plt.savefig("/Users/smgroves/Documents/GitHub/Cahn_Hilliard_Model/Cahn_Hilliard_solvers/plotting/manuscript/figure_4/interface.pdf")
