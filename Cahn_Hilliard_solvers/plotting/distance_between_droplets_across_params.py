...
sns.histplot(data= long_dist_df, x = 'distance', palette=sns.color_palette("muted"), bins=30)
...
long_dist_df = make_long_dist_df(indir, file = "simulated_droplet_distances_e_0.0075.csv")
sns.histplot(data= long_dist_df, x = 'distance', palette=sns.color_palette("muted"), bins=30)
...
long_dist_df = make_long_dist_df(indir, file = "simulated_droplet_distances_e_0.0075_noisy_cohesin_sd_0.25.csv")
sns.histplot(data= long_dist_df, x = 'distance', palette=sns.color_palette("muted"), bins=30)
...
long_dist_df = make_long_dist_df(indir, file = "simulated_droplet_distances_e_0.0075_noisy_cohesin_sd_0.11.csv")
sns.histplot(data= long_dist_df, x = 'distance', palette=sns.color_palette("muted"), bins=30)
...
for d in all_images:
    long_dist_df = pd.concat([long_dist_df,pd.DataFrame({"Category":["Experiment"],"seed":[0],"time":[0],"cpc":[0], "cohesin":[0], "distance":[d]})], ignore_index=True)

#%%
# dist_df = pd.DataFrame(columns=['distance', 'chr'])
binwidth = 0.05
sns.histplot(data=long_dist_df,
             x='distance',
             hue='Category',
             binwidth=binwidth,
             stat="probability",
...
num_sim_timepoints = (len(long_dist_df['seed'].unique())-1)*(len(long_dist_df['cpc'].unique())-1)*(len(long_dist_df['cohesin'].unique())-1)*254
...
sns.histplot(data=long_dist_df.loc[long_dist_df['Category'] == 'Experiment'],
             x='distance',
             bins=30,
             stat="probability",
            binwidth=binwidth,
