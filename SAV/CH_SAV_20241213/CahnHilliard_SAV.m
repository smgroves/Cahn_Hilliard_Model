function [t_out, phi_t, mass_t, E_t] = CahnHilliard_SAV(phi0, varargin)
...
%NAME-VALUE PAIRS
    ...
    % filename = Name of the file to which phi is printed. Default = 'phi'.
%
%OUTPUT
    % t_out = Time corresponding to the dt time step outputs.
    % phi_t = Multidimensional array of phi over t_out.
    % mass_t = Vector of total mass over t_out.
...
%% Set option defaults and parse inputs
    ...
    addParameter(CahnHilliard_SAV_parser,'filename',default_filename,valid_string);
    ...
    filename = CahnHilliard_SAV_parser.Results.filename;
...
%% Run SAV solver
    ...
    for i = 1:t_iter
        ...
        % Store current states every t_out time steps
        if mod(i,dt_out) == 0
...
%% For post-processing
    ...
    if printphi
        ...
        writematrix(phi_new, Path, 'WriteMode', 'append'); 
