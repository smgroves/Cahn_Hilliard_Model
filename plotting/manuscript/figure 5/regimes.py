...
indir = "/Users/smgroves/Documents/GitHub/Cahn_Hilliard_Model/plotting/manuscript/figure 5"
df = pd.read_csv(
    f"{indir}/alpha_0.0_CPC_droplet_regimes_e_0.0067.csv", header=0, index_col=None)

# df['CPC(unitless)'] = df['CPC(unitless)']*3.2
# df['cohesin(unitless)'] = df['cohesin(unitless)']*3.2

plt.figure(figsize=(6, 4), dpi=300)
ax = sns.scatterplot(data=df, x="CPC(unitless)",
                     y="cohesin(unitless)", hue="dynamics")
...
plt.tight_layout()
# plt.show()
plt.savefig(f"{indir}/regimes_0.0067.pdf")
