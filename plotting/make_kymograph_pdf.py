...
def add_images_to_pdf_sorted_grid(image_paths, pdf_path, title):
    ...
    margin = 30
    padding = 0
    img_width = (page_width - 2 * margin - (len(unique_eps) - 1) * padding) / len(
        unique_eps
    )

    # Calculate the positions for each unique CPC and alpha value
    eps_to_x = {
        eps: margin + i * (img_width + padding) for i, eps in enumerate(unique_eps)
    }
    alpha_to_y = {
        alpha: page_height - margin - (i * (img_width + padding))
...
indir = f"/Users/smgroves/Documents/GitHub/Cahn_Hilliard_Model/nonlinear_multigrid/plotting/radii_over_time_level_set_plots/{sim}/"
paths = glob(f"{indir}/*/")
image_paths = []
for p in paths:
...
print(image_paths)
pdf_path = f"/Users/smgroves/Documents/GitHub/Cahn_Hilliard_Model/nonlinear_multigrid/plotting/kymograph_pdfs/{sim}.pdf"
