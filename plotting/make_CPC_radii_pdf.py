...
def add_images_to_pdf_sorted_grid(image_paths, pdf_path):
    ...
    margin = 30
    padding = 10
    img_width = (page_width - 2 * margin - (len(unique_cpc) - 1) * padding) / len(
        unique_cpc
    )

    # Calculate the positions for each unique CPC and cohesin value
    cpc_to_x = {
        cpc: margin + i * (img_width + padding) for i, cpc in enumerate(unique_cpc)
    }
    cohesin_to_y = {
        cohesin: page_height - margin - (i * (img_width + padding))
        for i, cohesin in enumerate(unique_cohesin)
    }
...
# paths = glob(f"{indir}/radii_lineplots_kymographs/alpha_-0.2/*/")
# image_paths = []
# epsilon = "0.0125"
# for p in paths:
#     print(p)
...
print(image_paths)
# pdf_path = f"{indir}/radii_over_time_pdfs/radii_over_time_eps_{epsilon}_alpha_-0.2.pdf"
pdf_path = f"{indir}/kymograph_pdfs/kymographs_{sim}.pdf"
# add_images_to_pdf(image_paths, pdf_path)
