...
int i,j;
double x,y;
ijloop {x=(i-0.5)*h; y=(j-0.5)*h;phi[i][j]=cos(PI*x)*cos(PI*y);} /* Sinusoidal input for convergence test */
}
...
xleft=0.0; xright=1.0; yleft=0.0; yright=1.0; max_it=100; ns=1; dt=0.01; /* Set x-y dimenison, max iterations, and number of steps before printing results */
h=xright/(double)nx; h2=pow(h,2); gam=0.06; Cahn=pow(gam,2); /* gam is gradient interfacial energy; Page 1 of Mathematics 8:97 (2020) */
