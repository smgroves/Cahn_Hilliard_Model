...
int i,j,receptor,activation;
double x,y;
srand (time(NULL)); /* Initialize pseudorandom number generator seed */
receptor=1; /* rate^-1 of encountering a receptor per grid point */
activation=16; /* rate^-1 of encountering an active receptor per grid point */
ijloop {x=(i-0.5)*h; y=(j-0.5)*h; 
/*phi[i][j]=cos(PI*x)*cos(PI*y);*/ /* Sinusoidal input for convergence test */
if (rand()%activation == 0) phi[i][j]=1; /* 0.1*(1-2*rand()%10); */
else if (rand()%receptor == 0) phi[i][j]=0;
else phi[i][j]=-1;
}
}
...
xleft=0.0; xright=1.0; yleft=0.0; yright=1.0; max_it=500; ns=10; /* Set x-y dimenison, max iterations, and number of steps before printing results */
h=xright/(double)nx; h2=pow(h,2); gam=4*h/(2*sqrt(2)*atanh(0.9)); Cahn=pow(gam,2); dt=0.1*h2; /* gam is gradient interfacial energy; Page 1 of Mathematics 8:97 (2020) */
