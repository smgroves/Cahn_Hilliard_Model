...
if (i > round(nx/2)-10 && i < round(nx/2)+10) {
    if (j > round(ny/2)-10 && j < round(ny/2)+10) {phi[i][j]=1;}
    else if (i > round(nx/2)-2 && i < round(nx/2)+2) {phi[i][j]=1;}
    else {phi[i][j]=-1;}
}
else {phi[i][j]=-1;}
}
...
xleft=0.0; xright=1.0; yleft=0.0; yright=1.0; max_it=600; ns=5; /* Set x-y dimenison, max iterations, and number of steps before printing results */
