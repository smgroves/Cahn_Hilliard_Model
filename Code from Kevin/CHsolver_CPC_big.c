...
double **dmatrix(long nrl,long nrh,long ncl,long nch){ /* Discrete matrix passed:
...
m=(double **) malloc((nrow)*sizeof(double*)); m+=1;m-=nrl; /* Allocate matrix rows starting at nrl */
m[nrl]=(double *) malloc((nrow*ncol)*sizeof(double)); m[nrl]+=1; m[nrl]-=ncl; /* Allocate matrix columns starting at ncl */
for (i=nrl+1; i<=nrh; i++) m[i]=m[i-1]+ncol; /* Add columns to each row from nrl+1 to nrh */
return m;
}

void free_dmatrix(double **m,long nrl,long nrh,long ncl,long nch){
free(m[nrl]+ncl-1); free(m+nrl-1); /* Frees up pointers created by malloc */
}

void zero_matrix(double **a,int xl,int xr,int yl,int yr){ /* Creates zero matrix from xl –> xr and yl –> yr */
int i,j;
for (i=xl; i<=xr; i++){ for (j=yl; j<=yr; j++){ a[i][j]=0.0; }}
}

void mat_add2(double **a,double **b,double **c,double **a2, 
double **b2,double **c2,int xl,int xr,int yl,int yr){ /* Adds two matrices a = b + c and a2 = b2 + c2
...
double **b2,double **c2,int nrl,int nrh,int ncl,int nch){ /* Subtracts two matrices a = b - c and a2 = b2 - c2
...
void mat_copy(double **a,double **b,int xl,int xr,int yl,int yr){ /* Copies matrix b to matrix a
from xl –> xr and yl –> yr */
int i,j;
for (i=xl; i<=xr; i++){ for (j=yl; j<=yr; j++){ a[i][j]=b[i][j]; }}
}

void mat_copy2(double **a,double **b,double **a2,double **b2,int xl,int xr,int yl,int yr){ /* Copies matrix b to matrix a
...
void print_mat(FILE *fptr,double **a,int nrl,int nrh,int ncl,int nch){ /* Prints matrix a in a space-delimited format 
from nrl –> nrh and ncl –> nch */
int i,j;
...
void print_data(double **phi) { /* Prints the conserved scalar field phi as a space-delimited file */
FILE *fphi;
fphi=fopen("phi.m","a"); print_mat(fphi,phi,1,nx,1,ny); fclose(fphi); /* Appends to an existing file */
}

void laplace(double **a,double **lap_a,int nxt,int nyt){ /* Calculate discrete Laplace operator of matrix a, 
...
ijloopt {
if (i>1) dadx_L=a[i][j]-a[i-1][j]; /* Calculate left differential */
else dadx_L=0.0; /* Neumann boundary conditions */
if (i<nxt) dadx_R=a[i+1][j]-a[i][j]; /* Calculate right differential */
else dadx_R=0.0; /* Neumann boundary conditions */
if (j>1) dady_B=a[i][j]-a[i][j-1]; /* Calculate bottom differential */
else dady_B=0.0; /* Neumann boundary conditions */
if (j<nyt) dady_T=a[i][j+1]-a[i][j]; /* Calculate top differential */
else dady_T=0.0; /* Neumann boundary conditions */
lap_a[i][j]=(dadx_R-dadx_L+dady_T-dady_B)/ht2;} /* Defined just below Equation 6 in Mathematics 8:97 (2020) */
}

void source(double **c_old,double **src_c,double **src_mu){ /* Calculates the spatial residuals of phi and mu as source terms to the smoothing operator
...
ijloop{src_c[i][j]=c_old[i][j]/dt-ct[i][j]; src_mu[i][j]=0.0;} /* Update spatial residual of phi and set spatial residual of mu to zero */
}
...
int nxt, int nyt){ /* Relaxation operator passing new phi, new mu, 𝝵 source term, ψ source term, [ilevel multigrid index appears to be 
passed for no reason] */
int i,j,iter;
double ht2,x_fac,y_fac,a[4],f[2],det;
ht2=pow((xright-xleft)/(double) nxt,2); /* Space step size squared [~THIS ASSUMES A SQUARE MESH GRID... NOTE THAT nyt IS NOT USED~] */
for (iter=1; iter<=c_relax; iter++){ /* For each relaxation cycle */
ijloopt {
if (i>1 && i<nxt) x_fac=2.0;
else x_fac=1.0;
if (j>1 && j<nyt) y_fac=2.0;
else y_fac=1.0;
a[0]=1.0/dt; a[1]=(x_fac+y_fac)/ht2; /* First- and second-term coefficients of Equation 22 in Mathematics 8:97 (2020) */
a[2]=-(x_fac+y_fac)*Cahn/ht2-d2f(c_new[i][j]); a[3]=1.0; /* First- and second-term coefficients of Equation 23 in Mathematics 8:97 (2020) */
f[0]=su[i][j]; f[1]=sw[i][j]+df(c_new[i][j])-d2f(c_new[i][j])*c_new[i][j]; /* Third term of Equation 22 and third and fourth terms 
...
mu_new[i][j]=(-a[2]*f[0]+a[0]*f[1])/det; }} /* Solve for the next mu */
}

void restrictCH(double **uf,double **uc,double **vf,double **vc,int nxc,int nyc) { /* Restrict the defect twofold in each direction,
...
void nonL(double **ru,double **rw,double **c_new,double **mu_new,int nxt,int nyt) { /* NSO operator just above Equation 19 of Mathematics 8:97 (2020) */
int i,j;
double **lap_mu,**lap_c;
lap_mu=dmatrix(1,nxt,1,nyt); lap_c=dmatrix(1,nxt,1,nyt); /* Create discrete matrices of ones */
laplace(c_new,lap_c,nxt,nyt); laplace(mu_new,lap_mu,nxt,nyt); /* Initialize discrete Laplace operator */
ijloopt{ ru[i][j]=c_new[i][j]/dt-lap_mu[i][j]; /* Apply NSO to phi */
rw[i][j]=mu_new[i][j]-df(c_new[i][j])+Cahn*lap_c[i][j]; } /* Apply NSO to mu */
free_dmatrix(lap_mu,1,nxt,1,nyt); free_dmatrix(lap_c,1,nxt,1,nyt); /* Free up pointers */
}

void defect(double **duc,double **dwc,double **uf_new,double **wf_new,double **suf,
double **swf,int nxf,int nyf,double **uc_new,double **wc_new,int nxc,int nyc) { /* Compute defect */
double **ruf,**rwf,**rruf,**rrwf,**ruc,**rwc;
ruc=dmatrix(1,nxc,1,nyc);rwc=dmatrix(1,nxc,1,nyc);ruf=dmatrix(1,nxf,1,nyf); /* Initialized discrete matrices */
...
free_dmatrix(ruc,1,nxc,1,nyc); free_dmatrix(rwc,1,nxc,1,nyc); /* Free up pointers */
free_dmatrix(ruf,1,nxf,1,nyf); free_dmatrix(rwf,1,nxf,1,nyf);
free_dmatrix(rruf,1,nxc,1,nyc); free_dmatrix(rrwf,1,nxc,1,nyc);
}

void prolong_ch(double **uc,double **uf,double **vc,double **vf, int nxc,int nyc){ /* Expand twofold in each direction,
...
int ilevel) { /* FAS multigrid cycle */
relax(uf_new,wf_new,su,sw,ilevel,nxf,nyf); /* Relax the input data */
if (ilevel<n_level) { /* If the number of multigrid levels has not been reached */
int nxc,nyc;
double **duc,**dwc,**uc_new,**wc_new,**uc_def,**wc_def,**uf_def,**wf_def;
nxc=nxf/2; nyc=nyf/2; /* Coarsen grid by twofold */
duc=dmatrix(1,nxc,1,nyc); dwc=dmatrix(1,nxc,1,nyc); /* Initialize matrices */
...
free_dmatrix(duc,1,nxc,1,nyc); free_dmatrix(dwc,1,nxc,1,nyc); /* Free up pointers */
...
double error2(double **c_old,double **c_new,double **mu,int nxt,int nyt){ /* Calculates 2D residual for phi */
int i,j;
...
void initialization(double **phi){ /* Initialize the domain for convergence test; Equation 26 of Mathematics 8:97 (2020) */
int i,j,CPC_width,cohesin_width;
double x,y;
srand (time(NULL)); /* Initialize pseudorandom number generator seed */
ijloop {x=(i-0.5)*h; y=(j-0.5)*h; 
...
else {phi[i][j]=-1;}*/ /* Setting to zero gives ringing artifacts */
CPC_width=20; /* best case = 20 */
cohesin_width=4; /* best case = 4 */
if (i > round(nx/2)-CPC_width && i < round(nx/2)+CPC_width) {
    if (j > round(ny/2)-CPC_width && j < round(ny/2)+CPC_width) {phi[i][j]=1;}
    else if (i > round(nx/2)-cohesin_width && i < round(nx/2)+cohesin_width) {phi[i][j]=1;}
    else {phi[i][j]=-1;}
}
else {phi[i][j]=-1;}
}
...
void cahn(double **c_old,double **c_new){ /* Cahn-Hilliard solver */
FILE *fphi2;
...
while (it_mg2<=max_it_CH && resid2>tol) { /* While max iterations have not been hit and residual is greater than tolerance */
it_mg2++; vcycle(c_new,mu,sc,smu,nx,ny,1); /* Update counter and run vcycle */
...
fprintf(fphi2,"%16.15f %d \n",resid2,it_mg2-1); fclose(fphi2);} /* [~NOT CLEAR WHY THIS STEP IS IN THE WHILE LOOP~] */
}
...
FILE *fphi,*fphi2;
c_relax=2; nx=gnx; ny=gny; n_level=(int)(log(nx)/log(2.0)+0.1); /* Set number of relaxation cycles and number of muligrid levels */
xleft=0.0; xright=1.0; yleft=0.0; yright=1.0; max_it=15000; ns=50; /* Set x-y dimenison, max iterations, and number of steps before printing results */
h=xright/(double)nx; h2=pow(h,2); gam=8*h/(2*sqrt(2)*atanh(0.9)); Cahn=pow(gam,2); dt=0.1*h2; /* gam [best = 8] is gradient interfacial energy; Page 1 of Mathematics 8:97 (2020) */
printf("nx=%d,ny=%d\n",nx,ny); printf("dt=%f\n",dt); /* Output model parameters */
printf("max_it=%d\n",max_it); printf("ns=%d\n",ns); printf("n_level=%d\n\n",n_level);
oc=dmatrix(0,nx+1,0,ny+1); nc=dmatrix(0,nx+1,0,ny+1); mu=dmatrix(1,nx,1,ny); /* Initialize matrices */
...
initialization(oc); mat_copy(nc,oc,1,nx,1,ny); /* Initialize oc and copy oc to nc */
fphi=fopen("phi.m","w"); fclose(fphi); print_data(oc); /* Save initial conditions by opening a new writeable file and appending with print_data */
for (it=1; it<=max_it; it++) { /* Run Cahn-Hilliard solver */
cahn(oc,nc); mat_copy(oc,nc,1,nx,1,ny); /* Run one iteration of Cahn-Hilliard and copy oc to nc */
if (it % ns==0) {count++; print_data(oc); printf("print out counts %d \n",count);} /* Every ns iterations print out counts to phi.m*/
printf(" %d \n",it);}
return 0;
}