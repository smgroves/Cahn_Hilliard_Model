...
function nonL!(lap_c, lap_mu, c_new, mu_new, nxt, nyt, dt, Cahn)
    ru = zeros(Float64, nxt, nyt)
    rw = zeros(Float64, nxt, nyt)
    laplace!(lap_c, c_new, nxt, nyt)
    laplace!(lap_mu, mu_new, nxt, nyt)
...
function defect!(lap_c, lap_mu, uf_new, wf_new, suf, swf, nxf, nyf, uc_new, wc_new, nxc, nyc, dt, Cahn)
    ruc, rwc = nonL!(lap_c, lap_mu, uc_new, wc_new, nxc, nyc, dt, Cahn)
    ruf, rwf = nonL!(lap_c, lap_mu, uf_new, wf_new, nxf, nyf, dt, Cahn)
    ruf = suf - ruf
    rwf = swf - rwf
...
function vcycle(uf_new, wf_new, su, sw, nxf, nyf, ilevel, c_relax, xright, xleft, dt, Cahn, n_level)
    relax!(uf_new, wf_new, su, sw, nxf, nyf, c_relax, xright,
        xleft, dt, Cahn)
    ...
    if ilevel < n_level
        ...
        duc, dwc = defect!(lap_c, lap_mu, uf_new, wf_new, su, sw, nxf, nyf, uc_new, wc_new, nxc, nyc, dt, Cahn)
...
function error2!(rr, c_old, c_new, mu, nxt, nyt, dt)
    x = 0.0

    rr .= mu .- c_old

    sor = zeros(Float64, nxt, nyt)
    laplace!(sor, rr, nxt, nyt)
...
function cahn!(rr, c_old, c_new, mu, nx, ny, dt, max_it_CH, tol, c_relax, xright, xleft, Cahn, n_level; suffix="", print_r=true)
    it_mg2 = 0
    resid2 = 1
    ...
    while resid2 > tol && it_mg2 < max_it_CH
        ...
        resid2 = error2!(rr, c_old, c_new, mu, nx, ny, dt)
        if print_r
...
function multigrid_solver(oc, nx, tol, outdir; max_it=1000, max_it_CH=10000, suffix="", overwrite=true, print_phi=true, print_mass=false, print_e=false, print_r=true, dt=2.5e-5, m=8, ns=10, epsilon=0.0, check_dir=true)
    while true
        ...
        #run cahn hilliard
        for it in 1:max_it
            cahn!(rr, oc, nc, mu, nx, ny, dt, max_it_CH, tol, c_relax, xright, xleft, Cahn, n_level, suffix=suffix, print_r=print_r)
