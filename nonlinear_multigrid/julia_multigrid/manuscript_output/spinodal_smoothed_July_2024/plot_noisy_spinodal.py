...
indir = "/Users/smgroves/Documents/GitHub/Cahn_Hilliard_Model/finite_difference_method/output/spinodal_MG_timepoint_IC/t=0.01"
...
data = np.loadtxt(
    f"{indir}/FD_1600_dt_6.25e-06_Nx_128_gam_3.69e+00_D_16384_phi.csv", delimiter=","
)
# data = np.loadtxt(
...
for i in range(nx):  # Iterate over X
    for j in range(nx):  # Iterate over Y
        ...
        plt.plot(reshaped_data[:, i, j], linewidth=0.5, alpha=0.5, color=color)
        plt.xlabel("Timestep")
        plt.ylabel("Phi")
# plt.axhline(y=mean, linestyle="--", color="grey")
plt.axhline(y=-0.58, linestyle="--", color="black")
plt.axhline(y=0.58, linestyle="--", color="black")
plt.axhline(y=0, linestyle="--", color="black")

# plt.title(f"Evolution of spinodal decomposition with IC mean = {mean}, sd = {sd}")
sm = cm.ScalarMappable(cmap=colormap, norm=norm)

sm.set_array([])
# plt.colorbar(sm, label=f"{color_decider} Value")
# plt.show()
plt.savefig(f"{indir}/lineplot_{color_decider}_range.png")
